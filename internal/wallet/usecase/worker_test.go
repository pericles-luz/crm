...
// TestWorker_EscalatesAfterThreshold asserts AC #3: after N failed
// attempts the worker emits the persistent-failure alert.
func TestWorker_EscalatesAfterThreshold(t *testing.T) {
	...
	w := usecase.ReconcileWorker{
		...
		Clock: clock, Backoffs: []time.Duration{1 * time.Millisecond, 1 * time.Millisecond},
		EscalateAfter: 5,
	}
	w.RunOnce(context.Background())
