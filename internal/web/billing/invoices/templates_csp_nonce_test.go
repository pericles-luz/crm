...
// billing/invoices template. Each render emits the
// <style id="tenant-theme"> with `nonce="{{$.CSPNonce}}"` so the strict
// `style-src 'self' 'nonce-…'` policy (no `'unsafe-inline'`) accepts
// the inline tag in the browser.
func TestLayouts_StampCSPNonce(t *testing.T) {
	t.Parallel()
	style := branding.DefaultThemeStyle
	const nonce = "test-csp-nonce-invoices"
	...
	cases := []struct {
	...
	}{
		{
			name: "list",
			tmpl: listLayoutTmpl,
			view: listView{TenantThemeStyle: style, CSPNonce: nonce},
		},
		{
			name: "detail",
