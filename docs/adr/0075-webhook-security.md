...
| C19 | §2 D7 — wording correction: `Nats-Msg-Id` declarado como `hex(eventID)` (não `hex(idempotency_key)`); nota explicando que reconciler retries republicam o mesmo `eventID` para a mesma row de `raw_event`, preservando F-14 dentro da janela `Duplicates: 1h`. Doc-only — alinha o ADR com a implementação landed em SIN-62234. | [SIN-62279](/SIN/issues/SIN-62279) (post-rev-3 amendment, doc-only) |
...
- **Garantia:** at-least-once para o consumer NATS. Duplicate publish (reconciler + race com publish original) é dedup'd pelo JetStream via header `Nats-Msg-Id` populado com **`hex(eventID)`**, onde `eventID` é o `raw_event.id` (uuid) gerado no INSERT de D6 step 4. Reconciler retries (D7 step 1) republicam o **mesmo** `eventID` para a mesma row pendente — portanto, dentro da janela `Duplicates: 1h` da stream config (ver bullet abaixo), JetStream descarta a duplicata, preservando a propriedade F-14 end-to-end. `eventID` é funcionalmente equivalente a `idempotency_key` para fins de dedup do publish path: `idempotency_key` é único por `(tenant, channel, raw_payload)` em `webhook_idempotency`, e cada chave dispara no máximo um INSERT em `raw_event` (D6 step 3 `ON CONFLICT DO NOTHING`), logo um único `eventID` por chave. Usar `eventID` (e não `idempotency_key`) no `Nats-Msg-Id` evita carregar a chave de `webhook_idempotency` no path do reconciler — `eventID` já está na row de `raw_event` que o reconciler está iterando, eliminando +1 round-trip por retry sem ganho de propriedade.
